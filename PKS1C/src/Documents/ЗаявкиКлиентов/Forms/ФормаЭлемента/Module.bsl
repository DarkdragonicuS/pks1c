...
Функция ЕстьПравоНаЗапись()
	...
	Если Не УправлениеДоступом.ЕстьРоль("асРедактированиеЧужихЗаявок") И Не (СтарыйИсполнитель = Справочники.ФизическиеЛица.ПустаяСсылка() ИЛИ СтарыйИсполнитель = Пользователи.ТекущийПользователь().ФизическоеЛицо) Тогда
		Возврат Ложь;
	Иначе
		Возврат Истина;
	КонецЕсли;
