...
Функция ПолучитьДанныеВыбораКонтактногоЛица(Текст)	
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|	ИЛИ КонтактныеЛицаПартнеровКонтактнаяИнформация.НомерТелефонаБезКодов ПОДОБНО &НомерТелефонаБезКодов)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	КонтактныеЛицаПартнеров.Наименование ПОДОБНО &Наименование
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		ПОЛНОЕ СОЕДИНЕНИЕ Телефоны КАК Телефоны
		|		ПО КонтактныеЛица.Ссылка = Телефоны.Ссылка
		|ГДЕ
		|	НЕ КонтактныеЛица.Ссылка.ПометкаУдаления
		|	ИЛИ НЕ Телефоны.Ссылка.ПометкаУдаления
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("НомерТелефонаБезКодов", "%" + Текст + "%");
	Запрос.УстановитьПараметр("НомерТелефона", "%" + Текст + "%");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		СписокВыбора.Добавить(ВыборкаДетальныеЗаписи.Ссылка, Строка(ВыборкаДетальныеЗаписи.Ссылка) + "(" + ВыборкаДетальныеЗаписи.НомерТелефона + ")");
	КонецЦикла;
