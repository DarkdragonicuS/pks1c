...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если ЗначениеЗаполнено(ПричинаЗаявкиВнеОчереди) Тогда
		Если ЗначениеЗаполнено(ПричинаЗаявкиВнеОчереди) Тогда
			ОчередьЗаявок = АутсорсРаботы.ОчередьЗаявок(Объект.ОтветственноеЛицо,Объект.Ссылка);
			ДополнительныеПараметры = Новый Структура("ОчередьЗаявок,Дата",ОчередьЗаявок,Объект.Дата);
			АутсорсРаботы.СохранитьПричинуЗаявкиВнеОчереди(Объект.Ссылка,Объект.ОтветственноеЛицо,ДополнительныеПараметры.ОчередьЗаявок,ПричинаЗаявкиВнеОчереди,ДополнительныеПараметры.Дата);
			ПричинаЗаявкиВнеОчереди = "";
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьДанныеВыбораКонтактногоЛица(Текст,Клиент=Неопределено)	
	...
	Пока ВыборкаКонтактныеЛица.Следующий() Цикл		
		...
		Результат = Строка(ВыборкаКонтактныеЛица.Ссылка);
		Итерация = 1;			
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			//более одного телефона перечисляем через запятую
			Если Итерация > 1 Тогда
				Результат = Результат + ", " + ВыборкаДетальныеЗаписи.НомерТелефона;	
			//открываем список		
			Иначе
				Результат = Результат + " (" + ВыборкаДетальныеЗаписи.НомерТелефона;
			КонецЕсли;
		...
		//телефоны у контактного лица есть, закрываем список
		Если Итерация > 1 Тогда
			Результат = Результат + ")"
		КонецЕсли;
		СписокВыбора.Добавить(ВыборкаКонтактныеЛица.Ссылка, ПодсветитьСлова(Результат,МассивСловПоиска));
	КонецЦикла;
...
Процедура КонтактноеЛицоОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Результат = ПолучитьКонтактыЛица(ВыбранноеЗначение);
	СписокТелефонов = Результат.СписокТелефонов;
	СписокEmail = Результат.СписокEmail;
	
	Если СписокТелефонов.Количество() = 1 Тогда
		Объект.КонтактныйТелефон = СписокТелефонов[0].Значение;
	ИначеЕсли СписокТелефонов.Количество() > 1 Тогда
		ОповещениеПослеВыбораЭлемента = Новый ОписаниеОповещения(
		    "ВыбратьТелефон", ЭтотОбъект);	
		СписокТелефонов.ПоказатьВыборЭлемента(ОповещениеПослеВыбораЭлемента, "Выберите контактный телефон");
	КонецЕсли;
	
	Если СписокEmail.Количество() = 1 Тогда
		Объект.Почта = СписокEmail[0].Значение;
	ИначеЕсли СписокEmail.Количество() > 1 Тогда
		ОповещениеПослеВыбораЭлемента = Новый ОписаниеОповещения(
		    "ВыбратьEmail", ЭтотОбъект);	
		СписокТелефонов.ПоказатьВыборЭлемента(ОповещениеПослеВыбораЭлемента, "Выберите контактный e-mail");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьТелефон(Элемент, Параметры) Экспорт
	Если Элемент <> Неопределено Тогда
		Объект.КонтактныйТелефон = Элемент.Значение;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьEmail(Элемент, Параметры) Экспорт
	Если Элемент <> Неопределено Тогда
		Объект.Почта = Элемент.Значение;
	КонецЕсли;
КонецПроцедуры
...
//  Структура - контакты лица:
// * СписокТелефонов - СписокЗначений -
// * СписокEmail - СписокЗначений -
&НаСервере
Функция ПолучитьКонтактыЛица(КонтактноеЛицо)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Если ВыборкаДетальныеЗаписи.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон Тогда
			СписокТелефонов.Добавить(ВыборкаДетальныеЗаписи.Представление);
		ИначеЕсли ВыборкаДетальныеЗаписи.Тип = Перечисления.ТипыКонтактнойИнформации.АдресЭлектроннойПочты И ВыборкаДетальныеЗаписи.Вид = Справочники.ВидыКонтактнойИнформации.EmailКонтактногоЛица Тогда
			СписокEmail.Добавить(ВыборкаДетальныеЗаписи.Представление);
		КонецЕсли;
	КонецЦикла;
	
	Возврат Новый Структура("СписокТелефонов,СписокEmail",СписокТелефонов,СписокEmail);
КонецФункции
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	//Контроль очереди заявок
	Если Не РазрешеноИгнорированиеОчередиЗаявок() Тогда
		Если Не Параметры.Ключ.Пустая() И ЗначениеЗаполнено(Объект.ОтветственноеЛицо) И Не ЗначениеЗаполнено(ПричинаЗаявкиВнеОчереди) Тогда
			Если Объект.ОтветственноеЛицо <> ИсполнительДоЗаписи() Тогда
				Если Объект.Приоритет <> ПредопределенноеЗначение("Перечисление.ПриоритетыЗаявок.Срочно") Тогда
					ОчередьЗаявок = АутсорсРаботы.ОчередьЗаявок(Объект.ОтветственноеЛицо,Объект.Ссылка);
					Если ОчередьЗаявок.Количество() <> 0 Тогда
						//Вывести форму со списком заявок и требованием причины игнорирования
						ПараметрыФормы = Новый Структура("ОчередьЗаявок",ОчередьЗаявок);
						ДополнительныеПараметры = Новый Структура("ОчередьЗаявок,Дата",ОчередьЗаявок,Дата);
						ОткрытьФорму("Документ.ЗаявкиКлиентов.Форма.ФормаОчередиЗаявок", ПараметрыФормы, ЭтотОбъект, , , , Новый ОписаниеОповещения("ПослеЗакрытияФормыОчередиЗаявок", ЭтотОбъект, ДополнительныеПараметры), РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
						Отказ = Истина;
						Возврат;
					КонецЕсли;
				КонецЕсли;
...
&НаСервереБезКонтекста
Функция РазрешеноИгнорированиеОчередиЗаявок()
	Возврат УправлениеДоступом.ЕстьРоль("асИгнорированиеОчередиЗаявок");
КонецФункции

&НаКлиенте
Процедура ПослеЗакрытияФормыОчередиЗаявок(РезультатОбработки, ДополнительныеПараметры) Экспорт
	Если ТипЗнч(РезультатОбработки) = Тип("Структура") И РезультатОбработки.Свойство("Причина") Тогда 
		ПричинаЗаявкиВнеОчереди = РезультатОбработки.Причина;
		Записать();
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция ИсполнительДоЗаписи()
	Возврат Объект.Ссылка.ОтветственноеЛицо;
КонецФункции
