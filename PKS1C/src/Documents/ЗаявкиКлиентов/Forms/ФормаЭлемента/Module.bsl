...
Функция ПолучитьДанныеВыбораКлиента(Текст,Контакт=Неопределено)	
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		КлиентСтрока = Новый Структура();
		КлиентСтрока.Вставить("ИсходнаСтрока",);
		КлиентСтрока.Вставить("ШрифтПодсветки",Новый Шрифт(,, Истина));
		КлиентСтрока.Вставить("ЦветПодсветки",Новый Цвет(0, 153, 0));
		
		КонтрагентСтрока = Новый Структура();
		КонтрагентСтрока.Вставить("ИсходнаяСтрока",ВыборкаДетальныеЗаписи.КонтрагентНаименование);
		КлиентСтрока.Вставить("ШрифтПодсветки",Новый Шрифт(,, Истина));
		КлиентСтрока.Вставить("ЦветПодсветки",Новый Цвет(0, 153, 0));
...
Функция ПолучитьДанныеВыбораКонтактногоЛица(Текст,Клиент=Неопределено)	
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		Запрос.Текст =
			...
			|ГДЕ
			|	КонтактныеЛицаПартнеров.Наименование ПОДОБНО &Текст
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
