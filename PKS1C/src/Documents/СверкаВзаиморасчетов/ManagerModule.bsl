...
Функция ВывестиДетальныеЗаписи(
	...
	Если НастройкиПечати.ДетализацияВзаиморасчетов = "ОбъектРасчетов"
		...
		ШаблонЗаголовка = ШаблонЗаголовка + " " + НСтр("ru = 'по объекту расчетов'") + " %1 ";
	ИначеЕсли НастройкиПечати.ДетализацияВзаиморасчетов = "Договор" И НЕ ЭтоФинансовыеИнструменты
		...
		ШаблонЗаголовка = ШаблонЗаголовка + " " + НСтр("ru = 'по договору'") + " %1 ";
	КонецЕсли;
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ (
	|		НЕ СверкаВзаиморасчетов.ПерезаполненыОстатки
	|		И НЕ СверкаВзаиморасчетов.ЕстьРасхождения
	|		И НЕ (ИСТИНА В
	|				(ВЫБРАТЬ ПЕРВЫЕ 1
	|					ИСТИНА
	|				ИЗ
	|					Документ.СверкаВзаиморасчетов.ДанныеКонтрагента КАК ДанныеКонтрагента
	|				ГДЕ
	|					СверкаВзаиморасчетов.Ссылка = ДанныеКонтрагента.Ссылка)
	|			)
	|	)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	СверкаВзаиморасчетов.ПерезаполненыОстатки КАК ПерезаполненыОстатки,
	|	ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	Пока Выборка.Следующий() Цикл
 		...
 		Попытка
			...
			Если НЕ Выборка.ПерезаполненыОстатки И НЕ Выборка.ЕстьДанныеКонтрагента И НЕ Выборка.ЕстьРасхождения Тогда
				КомпоновщикОтбор = Новый КомпоновщикНастроекКомпоновкиДанных;
				ИнициализироватьКомпоновщикНастроек(КомпоновщикОтбор);
