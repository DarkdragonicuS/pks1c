...
Процедура ЗаполнитьТаблицуРасчетовСервер()
	...
	Если ТаблицаРезультатаСКД.Количество() > 0 Тогда
		Группировки = "Контрагент, СуществуютДокументы, ИзменятьДокумент, СоздаватьДокумент, ДокументСверки";
		Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоОрганизаций") Тогда
			Группировки = "Организация, " + Группировки;
		КонецЕсли;
...
Процедура ОбновитьГиперссылкуОтбора(КнопкаРасширенногоОтбора, Отбор)
	...
	Для Каждого ЭлементОтбора Из КомпоновщикОтбор.Настройки.Отбор.Элементы Цикл
		...
		Если ЭлементОтбора.Использование
			И (ВидимыйОтборИзменен ИЛИ НЕ ВидимыОтбор) Тогда
			УстановленРасширенныйОтбор = Истина;
			Прервать;
		КонецЕсли;
