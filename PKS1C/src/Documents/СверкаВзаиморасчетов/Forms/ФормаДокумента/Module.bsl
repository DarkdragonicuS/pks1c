...
Процедура ДетальныеЗаписиРасчетыПриИзменении(Элемент)
	Элемент.ТекущиеДанные.ОбъектРасчетов = ОтборРасчеты;
КонецПроцедуры
...
Процедура УстановитьОтборДетализации(ПараметрыОтбора)
	...
	Отбор = Новый Структура(ПараметрыОтбора.ПолеОтбора, ПараметрыОтбора.ЗначениеОтбора);
	Элементы[ПараметрыОтбора.ТаблицаФормы].ОтборСтрок = Новый ФиксированнаяСтруктура(Отбор);
	
	ДеталиОбъекта = Новый Структура("Партнер, Договор", ПараметрыОтбора.Партнер, ПараметрыОтбора.Договор);
	Если ЗначениеЗаполнено(Объект.Партнер) Тогда
	...
	ПересчитатьИтогиПоОбъектуРасчетовНаСервере(ПараметрыОтбора.ТабличнаяЧасть, Отбор, ПараметрыОтбора.ПоляИтогов);
	УстановитьВидимость();
	
КонецПроцедуры

&НаСервере
Процедура УстановитьСледующийОбъект(ТекущийОтбор, ВОбработномНаправлении = Ложь)
	
	ВсеОбъекты = Объект[ТекущийОтбор.ТаблицаГруппировки].Выгрузить().ВыгрузитьКолонку(ТекущийОтбор.ПолеОтбора);
	Индекс = ВсеОбъекты.Найти(ТекущийОтбор.ЗначениеОтбора);
	НовыйИндекс = ?(ВОбработномНаправлении, Индекс-1, Индекс+1);
	Если НовыйИндекс < 0 Тогда
		НовыйИндекс = ВсеОбъекты.ВГраница();
	КонецЕсли;
	Если НовыйИндекс > ВсеОбъекты.ВГраница() Тогда
		НовыйИндекс = 0;
	КонецЕсли;
	ТекущийОтбор.ЗначениеОтбора = ВсеОбъекты[НовыйИндекс];
	Отбор = Новый Структура(ТекущийОтбор.ПолеОтбора, ТекущийОтбор.ЗначениеОтбора);
	НовыеДанные = Объект[ТекущийОтбор.ТаблицаГруппировки].НайтиСтроки(Отбор);
	Если НовыеДанные <> Неопределено Тогда
		Элементы[ТекущийОтбор.ТаблицаГруппировки].ТекущаяСтрока = НовыеДанные[0].ПолучитьИдентификатор();
		...
		Если НЕ ЗначениеЗаполнено(Объект.ТипРасчетов) Тогда
			ТекущийОтбор.ТипРасчетов = НовыеДанные[0].ТипРасчетов;
		КонецЕсли;
		ТекущийОтбор.Партнер = НовыеДанные[0].Партнер;
		ТекущийОтбор.Договор = НовыеДанные[0].Договор;
