...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.РегистрацияККТ") Тогда
		...
		ВерсияФФД = ДанныеЗаполнения.ВерсияФФД;
		КодПричиныПеререгистрацииККТФФД = ДанныеЗаполнения.КодПричиныПеререгистрацииККТФФД;
		СведенияОбИспользованииККТПоФФД = ДанныеЗаполнения.СведенияОбИспользованииККТПоФФД;
		ПризнакАгента = ДанныеЗаполнения.ПризнакАгента;
		НомерАвтомата = ДанныеЗаполнения.НомерАвтомата;
		АдресАУГород = ДанныеЗаполнения.АдресАУГород;
		АдресАУДом = ДанныеЗаполнения.АдресАУДом;
		АдресАУДомТип = ДанныеЗаполнения.АдресАУДомТип;
		АдресАУИндекс = ДанныеЗаполнения.АдресАУИндекс;
		АдресАУКвартира = ДанныеЗаполнения.АдресАУКвартира;
		АдресАУКвартираТип = ДанныеЗаполнения.АдресАУКвартираТип;
		АдресАУКорпус = ДанныеЗаполнения.АдресАУКорпус;
		АдресАУКорпусТип = ДанныеЗаполнения.АдресАУКорпусТип;
		АдресАУМестоПрименения = ДанныеЗаполнения.АдресАУМестоПрименения;
		АдресАУНаселенныйПункт = ДанныеЗаполнения.АдресАУНаселенныйПункт;
		АдресАУРайон = ДанныеЗаполнения.АдресАУРайон;
		АдресАУРегион = ДанныеЗаполнения.АдресАУРегион;
		АдресАУРегионНаименование = ДанныеЗаполнения.АдресАУРегионНаименование;
		АдресАУУлица = ДанныеЗаполнения.АдресАУУлица;
		
		Если ДанныеЗаполнения.ВидОперации = Перечисления.ВидыОперацийККТ.Закрытие Тогда
			ПричиныПеререгистрации = УстановитьБит(ПричиныПеререгистрации,4,Истина);
			КодПричиныПеререгистрацииККТФФД = УстановитьБит(КодПричиныПеререгистрацииККТФФД,0,Истина);
			ДокументЗакрытияФН = ДанныеЗаполнения.Ссылка;
		КонецЕсли;
	КонецЕсли;
