...
Процедура ПредварительныйПросмотрРегистрацииВККТ(Команда)
	...
	Если ВидОперации <> fptr.LIBFPTR_FNOP_CLOSE_ARCHIVE Тогда
		ТекстОтчета = ТекстОтчета + "Версия ФФД: " + Объект.ВерсияФФД + Символы.ПС;
	КонецЕсли;
	...
	Если ВидОперации = fptr.LIBFPTR_FNOP_CHANGE_FN И Объект.ВерсияФФД <> ПредопределенноеЗначение("Перечисление.ВерсииФФД.ФФД105") Тогда
		ТекстОтчета = ТекстОтчета + "Изменение сведений о ККТ: " + СписокПричинОПеререгистрацииСтрокой() + Символы.ПС;
	КонецЕсли;
	...
	Если ВидОперации <> fptr.LIBFPTR_FNOP_CLOSE_ARCHIVE Тогда
		...
		КонецЕсли;
			...
			КонецЕсли;
			Если Объект.ВерсияФФД = ПредопределенноеЗначение("Перечисление.ВерсииФФД.ФФД12") И Сведение120 Тогда
				ТекстОтчета = ТекстОтчета + "МАРКИРОВАННЫЙ ТОВАР" + Символы.ПС;
			КонецЕсли;
	КонецЕсли;
...
//			КонецЕсли;
	...
	Если fptr.isOpened() Тогда
		...
		Если ФНФискализирован Тогда
			...
			ВерсияФФД = fptr.getParamInt(1209);
			Маркировка = fptr.getParamBool(fptr.LIBFPTR_PARAM_TRADE_MARKED_PRODUCTS);
...
Процедура Сведение080ПриИзменении(Элемент)
	//СведениеФФД1126Лотереи = Сведение080;
	ЗаписьПризнаковРегистрации();
	ЭтаФорма.Модифицированность = Истина;
КонецПроцедуры
...
Процедура Сведение090ПриИзменении(Элемент)
	//СведениеФФД1193АзартныеИгры = Сведение090;
	ЗаписьПризнаковРегистрации();
	ЭтаФорма.Модифицированность = Истина;
КонецПроцедуры
...
Процедура Сведение110ПриИзменении(Элемент)
	//СведениеФФД1221ПринтерВАвтомате = Сведение110;
	ЗаписьПризнаковРегистрации();
	ЭтаФорма.Модифицированность = Истина;
КонецПроцедуры
...
Процедура Сведение130ПриИзменении(Элемент)
	//СведениеФФД1108Интернет = Сведение130;
	ЗаписьПризнаковРегистрации();
	ЭтаФорма.Модифицированность = Истина;
КонецПроцедуры
...
Процедура Сведение150ПриИзменении(Элемент)
	//СведениеФФД1109РасчетыЗаУслуги = Сведение150;
	ЗаписьПризнаковРегистрации();
	ЭтаФорма.Модифицированность = Истина;
КонецПроцедуры
...
Процедура Сведение155ПриИзменении(Элемент)
	//СведениеФФД1207ПодакцизныйТовар = Сведение155;
	ЗаписьПризнаковРегистрации();
	ЭтаФорма.Модифицированность = Истина;
КонецПроцедуры
...
Процедура Сведение120ПриИзменении(Элемент)
	//СведениеФФДМаркировка = Сведение120;
	ЗаписьПризнаковРегистрации();
	ЭтаФорма.Модифицированность = Истина;
КонецПроцедуры
...
Функция ЗаданиеJSONПолучитьПараметрыККТ()
	...
	ЗаданиеJSONПараметрыККТ.Вставить("internet",Сведение130);
	ЗаданиеJSONПараметрыККТ.Вставить("service",Ложь);
	ЗаданиеJSONПараметрыККТ.Вставить("excise",Сведение155);
	ЗаданиеJSONПараметрыККТ.Вставить("gambling",Сведение090);
	ЗаданиеJSONПараметрыККТ.Вставить("lottery",Сведение080);
	...
	ЗаданиеJSONПараметрыККТ.Вставить("marking",Сведение120);
...
Функция ПреобразоватьВерсиюФФДАтолВУчетную(ВерсияФФДАтол)
	...
	Если ВерсияФФДАтол = 105 Тогда
		ВерсияФФД = ПредопределенноеЗначение("Перечисление.ВерсииФФД.ФФД105");
	ИначеЕсли ВерсияФФДАтол = 110 Тогда
		ВерсияФФД = ПредопределенноеЗначение("Перечисление.ВерсииФФД.ФФД11");
	ИначеЕсли ВерсияФФДАтол = 120 Тогда
...
Процедура ЗаписьСведенийОбИспользованииККТвККТ()
	...
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,0,СведениеФФД1001АвтоматическийРежим);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,1,СведениеФФД1002АвтономныйРежим);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,2,СведениеФФД1056Шифрование);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,3,СведениеФФД1108Интернет);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,4,СведениеФФД1109РасчетыЗаУслуги);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,5,СведениеФФД1110БСО);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,6,СведениеФФД1126Лотереи);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,7,СведениеФФД1193АзартныеИгры);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,8,СведениеФФД1207ПодакцизныйТовар);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,9,СведениеФФД1221ПринтерВАвтомате);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,10,СведениеФФДЛомбарднаяДеятельность);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,11,СведениеФФДМаркировка);
	СведенияОбИспользованииККТ = УстановитьБит(СведенияОбИспользованииККТвККТ,12,СведениеФФДСтраховаяДеятельность);
