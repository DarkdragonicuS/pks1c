...
Процедура СформироватьЗаявление(Команда)
	...
	Если ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацийККТ.Закрытие") Тогда               //регистрация/перерегистрация
		//АдресМакета = ПоместитьВоВременноеХранилище(Документы.РегистрацияККТ.ПолучитьМакет("ЗаявлениеОРегистрации"));
		//Файл = ИмяФайлаЗаявления;
		//ПолучитьФайл(АдресМакет, Файл, Ложь);
		ЗаписьМакета(ИмяФайлаЗаявления,"ЗаявлениеОРегистрации");
		//КопироватьФайл("C:\Users\Cisco\Desktop\Регистрация ККТ заявлениеОБРАЗЕЦ.XLS", ИмяФайлаЗаявления);
		Попытка
			// Создать объект. 
		...
		Попытка
			...
			ЗаписьДанныхВКлеткиБланка(Лист, 11, 58, 1, 1, 1, 1, ?(ПроверитьБит(Объект.СведенияОбИспользованииККТ,1),"1","2"));
			ЗаписьДанныхВКлеткиБланка(Лист, 18, 58, 1, 1, 1, 1, ?(ПроверитьБит(Объект.СведенияОбИспользованииККТ,2),"1","2"));
			ЗаписьДанныхВКлеткиБланка(Лист, 23, 58, 1, 1, 1, 1, ?(ПроверитьБит(Объект.СведенияОбИспользованииККТ,10),"1","2"));
			ЗаписьДанныхВКлеткиБланка(Лист, 29, 58, 1, 1, 1, 1, ?(ПроверитьБит(Объект.СведенияОбИспользованииККТ,3),"1","2"));
			ЗаписьДанныхВКлеткиБланка(Лист, 32, 58, 1, 1, 1, 1, ?(ПроверитьБит(Объект.СведенияОбИспользованииККТ,4),"1","2"));
			ЗаписьДанныхВКлеткиБланка(Лист, 35, 58, 1, 1, 1, 1, ?(ПроверитьБит(Объект.СведенияОбИспользованииККТ,5),"1","2"));
			ЗаписьДанныхВКлеткиБланка(Лист, 39, 58, 1, 1, 1, 1, ?(ПроверитьБит(Объект.СведенияОбИспользованииККТ,6),"1","2"));
			...
			ЗаписьДанныхВКлеткиБланка(Лист, 10, 58, 1, 1, 1, 1, ?(ПроверитьБит(Объект.СведенияОбИспользованииККТ,7),"1","2"));
			ЗаписьДанныхВКлеткиБланка(Лист, 14, 58, 1, 1, 1, 1, ?(ПроверитьБит(Объект.СведенияОбИспользованииККТ,8),"1","2"));
			ЗаписьДанныхВКлеткиБланка(Лист, 18, 58, 1, 1, 1, 1, ?(ПроверитьБит(Объект.СведенияОбИспользованииККТ,9),"1","2"));
...
&НаСервере
Функция ФПД()
	Возврат Формат(Объект.ФискальныйПризнак,"ЧГ=");
КонецФункции

&НаСервере
Функция ФД()
	Возврат Формат(Объект.НомерФД,"ЧГ=");
КонецФункции

&НаСервере
Функция ФПДЗ()
	Возврат Формат(Объект.ДокументЗакрытияФН.ФискальныйПризнак,"ЧГ=");
КонецФункции

&НаСервере
Функция ФДЗ()
	Возврат Формат(Объект.ДокументЗакрытияФН.НомерФД,"ЧГ=");
КонецФункции
...
Процедура РегистрацияВККТ(Команда)	
	...
	Если fptr.isOpened() Тогда
		...
		fptr.fnQueryData();
		Объект.ФискальныйПризнак = fptr.getParamString(fptr.LIBFPTR_PARAM_FISCAL_SIGN);
		Объект.ДатаФормированияФП = fptr.getParamDateTime(fptr.LIBFPTR_PARAM_DATE_TIME);
		Объект.НомерФД = fptr.getParamInt(fptr.LIBFPTR_PARAM_DOCUMENT_NUMBER);
