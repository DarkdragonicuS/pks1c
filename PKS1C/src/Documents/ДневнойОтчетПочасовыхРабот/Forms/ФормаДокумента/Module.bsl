...
Процедура ДанныеПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если НоваяСтрока Тогда
		...
		Если Параметры.Ключ.Пустая() ИЛИ НачалоДня(Объект.Дата) = НачалоДня(ТекущаяДата) Тогда
			Элемент.ТекущиеДанные.ВремяПрибытия = Дата('00010101') + Час(ТекущаяДата) * 3600 + Минута(ТекущаяДата) * 60;
		КонецЕсли;
		Элемент.ТекущиеДанные.ВремяЗавершения = Дата('00010101');
		Элемент.ТекущиеДанные.Удаленка = Истина;
	КонецЕсли;
КонецПроцедуры
