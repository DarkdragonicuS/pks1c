...
Процедура ОбработкаПроведения(Отказ, Режим)	
	//Проверка корректности заполнения времени
	Для Каждого СтрокаРабота Из Данные Цикл
		Индекс = Данные.Индекс(СтрокаРабота);
		Если СтрокаРабота.ВремяПрибытия >= СтрокаРабота.ВремяЗавершения Тогда
			ОбщегоНазначения.СообщитьПользователю("Некорректно указан период работ!", ,"Данные[" + Формат(Индекс,"ЧН=0; ЧГ=;") + "].ВремяПрибытия" , "Объект");
			Отказ = Истина;
			Прервать;
		ИначеЕсли Не ЗначениеЗаполнено(СтрокаРабота.СоставРабот) Тогда
			ОбщегоНазначения.СообщитьПользователю("Не заполнен состав работ!", ,"Данные[" + Формат(Индекс,"ЧН=0; ЧГ=;") + "].СоставРабот" , "Объект");
			Отказ = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;
