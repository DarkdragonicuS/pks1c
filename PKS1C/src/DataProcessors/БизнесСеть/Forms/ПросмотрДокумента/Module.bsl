...
Процедура ЗагрузитьДокументЭДО()
	...
	Если СпособЗагрузкиДокумента = 0 Тогда
		Если Не ЗначениеЗаполнено(ТипОбъекта) Тогда
			Если Элементы.ТипОбъекта.СписокВыбора.Количество() Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Тип"" не заполнено'"),, "ТипОбъекта",, Отказ);
				Возврат;
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Загрузка данного документа не поддерживается'"),,,, Отказ);
				Возврат;
			КонецЕсли;
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(СписокНеСопоставленнойНоменклатуры) Тогда
		...
		Настройки.Вставить("Заголовок", СтрШаблон(НСтр("ru = 'Сопоставление номенклатуры для %1'"), ЭтотОбъект.Представление));
