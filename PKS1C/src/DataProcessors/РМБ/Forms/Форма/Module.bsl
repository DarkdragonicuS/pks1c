...
Процедура СформироватьОтчет()
	...
	Пока ВыборкаРабот.Следующий() Цикл
		...
		//	ОбластьСтроки.Параметры.Сумма = ВыборкаЦен.ТарифСверхурочный * ВыборкаРабот.ВремяЗатраты;	
		//Иначе 
...
&НаКлиенте
Процедура СформироватьТаблицуСчетов(Команда)
	СформироватьТаблицуСчетовНаСервере();
	ОбновитьТекстВыставленоСчетов();
КонецПроцедуры

&НаСервере
Процедура СформироватьТаблицуСчетовНаСервере()
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ДоговораОбслуживанияСрезПоследних.Контрагент КАК Контрагент,
		|	ДоговораОбслуживанияСрезПоследних.Контрагент.Наименование КАК Наименование,
		|	&От КАК Дата
		|ПОМЕСТИТЬ СПовторениями
		|ИЗ
		|	РегистрСведений.ДоговораОбслуживания.СрезПоследних(&От, ) КАК ДоговораОбслуживанияСрезПоследних
		|ГДЕ
		|	ДоговораОбслуживанияСрезПоследних.АбонентскаяПлата
		|	И НЕ ДоговораОбслуживанияСрезПоследних.ЧерныйСписок
		|	И (ДоговораОбслуживанияСрезПоследних.СрокДействия > &От
		...
		|	РегистрСведений.ПочасовыеРаботы КАК ПочасовыеРаботы
		|ГДЕ
		|	ПочасовыеРаботы.Период МЕЖДУ &От И &До
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	СПовторениями.Контрагент КАК Контрагент,
		|	СПовторениями.Наименование КАК Наименование,
		|	СПовторениями.Дата КАК Дата
		|ПОМЕСТИТЬ БезПовторений
		|ИЗ
		|	СПовторениями КАК СПовторениями
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	БезПовторений.Контрагент КАК Контрагент,
		|	БезПовторений.Наименование КАК Наименование,
		|	ЕСТЬNULL(ВыставленныеСчета.Выставлен, ЛОЖЬ) КАК СчетВыставлен,
		|	ЕСТЬNULL(ВыставленныеСчета.Период, БезПовторений.Дата) КАК Дата
		|ИЗ
		|	БезПовторений КАК БезПовторений
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВыставленныеСчета КАК ВыставленныеСчета
		|		ПО БезПовторений.Контрагент = ВыставленныеСчета.Контрагент
		|ГДЕ
		|	ЕСТЬNULL(ВыставленныеСчета.Период, БезПовторений.Дата) МЕЖДУ &От И &До
		|
		|УПОРЯДОЧИТЬ ПО
		|	Наименование";
	
	Запрос.УстановитьПараметр("От", НачалоМесяца(Период));
	Запрос.УстановитьПараметр("До", КонецМесяца(Период));
	
	РезультатЗапроса = Запрос.Выполнить();
	
	ТаблицаСчетов.Загрузить(РезультатЗапроса.Выгрузить());
КонецПроцедуры
