...
Процедура СформироватьНаСервере(ТабДок, ВидОтчета)
	...
	Пока ВыборкаДат.Следующий() Цикл
		...
		Если (ВыборкаДат.RSAДо - ТекущаяДата() >= 30 * 24 * 60 * 60 ИЛИ Не ЗначениеЗаполнено(ВыборкаДат.RSAДо)) И (ВыборкаДат.ГОСТДо - ТекущаяДата() >= 30 * 24 * 60 * 60 ИЛИ Не ЗначениеЗаполнено(ВыборкаДат.ГОСТДо)) Тогда
