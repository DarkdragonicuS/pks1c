...
Процедура СформироватьНаСервере(ТабДок, Исполнитель, Месяц)	
	Макет = Отчеты.ЗаработаноСотрудником.ПолучитьМакет("Макет");	
	...
	Пока ВыборкаРабот.Следующий() Цикл
		...
		//	Продолжить;	
		ИначеЕсли ВыборкаЦен.АбонентскаяПлата И НЕ ВыборкаРабот.Сверхурочка Тогда
			Продолжить;
		КонецЕсли;
		...
		Если Договор Тогда
			...
			Если ВыборкаРабот.Сверхурочка И ВыборкаЦен.ТарифСверхурочный <> 0 Тогда
				...
				ОбластьСтрока.Параметры.Сумма = ВыборкаЦен.ТарифСверхурочный * ВыборкаРабот.ВремяЗатраты;	
			Иначе
				//Если ОбластьСтрока.Параметры.АбонентскаяПлата Тогда
				//	ОбластьСтрока.Параметры.Сумма = Цел(ПосчитатьТариф(ВыборкаРабот.Организация, ВыборкаРабот.Дата, Месяц, КонецМесяца(Месяц)) * ВыборкаРабот.ВремяЗатраты);
				//Иначе
					ОбластьСтрока.Параметры.Сумма = ВыборкаЦен.Тариф * ВыборкаРабот.ВремяЗатраты;
				//КонецЕсли;
			КонецЕсли;
		Иначе
