	...
	Запрос.Текст = 
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Не Выборка.ИспользоватьСерии Тогда
				...
				ДанныеГенерации.ТекстОшибки = СтрШаблон(НСтр("ru = 'Для номенклатуры %1 серии не используются.'"), Выборка.НоменклатураНаименование);
				
			ИначеЕсли НЕ Выборка.АвтоматическиГенерироватьСерии
			 		И Не ЗначениеЗаполнено(Выборка.Серия) Тогда
			 	
			 	ДанныеГенерации.ЕстьОшибка = Истина;
				ДанныеГенерации.ТекстОшибки = СтрШаблон(НСтр("ru = 'Для номенклатуры %1 не предусмотрена автоматическая генерация серий.'"), Выборка.НоменклатураНаименование);
