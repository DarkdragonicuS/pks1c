...
// Возвращает список e-mail контактного лица в виде строки.
// 
// Параметры:
//  КонтактноеЛицо - СправочникСсылка.КонтактныеЛицаПартнеров - Контактное лицо
// 
// Возвращаемое значение:
//  Строка - e-mail контактного лица строкой
Функция EmailКонтактногоЛицаСтрокой(КонтактноеЛицо) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	email = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("АдресЭП");
	
	Возврат СтрСоединить(email,", "); 
КонецФункции

// Возвращает список клиентов контактного лица в виде строки.
// 
// Параметры:
//  КонтактноеЛицо - СправочникСсылка.КонтактныеЛицаПартнеров - Контактное лицо
// 
// Возвращаемое значение:
//  Строка - Клиенты контактного лица строкой
