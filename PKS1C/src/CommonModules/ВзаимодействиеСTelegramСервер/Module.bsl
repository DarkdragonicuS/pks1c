	...
	Если КоличествоЗаписей > 0 Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Иначе
				...
				ПараметрыКнопки.Вставить("callback_data",ПолучитьДанныеКомандыGetLicense(3,,,Номенклатура,ВыборкаДетальныеЗаписи.Ссылка,,Отправитель.id));
				МассивКнопок.Добавить(ПараметрыКнопки);
				МассивСтрокКнопок.Добавить(МассивКнопок);
			КонецЕсли;
		КонецЦикла;        	
		...
		Если Страница < КоличествоЗаписей / 10 Тогда
			ПараметрыКнопки = Новый Структура();
			ПараметрыКнопки.Вставить("text",">");
			ПараметрыКнопки.Вставить("callback_data",ПолучитьДанныеКомандыGetLicense(1,Клиент,Страница+1,Номенклатура,,,Отправитель.id));
			МассивКнопок.Добавить(ПараметрыКнопки);
		КонецЕсли;
		...
		ПараметрыКнопки = Новый Структура();
		ПараметрыКнопки.Вставить("text",">>");
		ПараметрыКнопки.Вставить("callback_data",ПолучитьДанныеКомандыGetLicense(1,Клиент,ОкруглитьВерх(КоличествоЗаписей / 10),Номенклатура,,,Отправитель.id));
		МассивКнопок.Добавить(ПараметрыКнопки);				
