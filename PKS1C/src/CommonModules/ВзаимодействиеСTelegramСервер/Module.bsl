...
Функция РазослатьЗаявку(Заявка,Параметры,Токен=Неопределено,Подключение=Неопределено) Экспорт
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		Для Каждого ПолучательID Из СписокПолучателей Цикл
			ТекстЗапроса = "/bot"+Токен + "/sendmessage?chat_id=" + Формат(ПолучательID,"ЧГ=") + "&text=" + ЭкранированиеСимволовQueryString(СообщениеИсполнителю);
			...
			Если Не Контакт = Неопределено Тогда
				Возврат ОтправитьКонтактВЧат(Токен,Контакт,ПолучательID,Подключение);
			КонецЕсли;
		КонецЦикла;
