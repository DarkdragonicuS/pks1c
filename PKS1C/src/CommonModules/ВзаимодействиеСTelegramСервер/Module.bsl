...
Функция РазослатьЗаявку(Заявка,НоваяЗаявка=Истина,Токен=Неопределено,Подключение=Неопределено) Экспорт
	Ошибка = Ложь;
	Ответ = "";
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		Сообщение = ?(НоваяЗаявка,"Получена новая заявка:","Обновлена заявка:")
		...
		ТекстОтвета = Сообщение;
	
		СписокПолучателей = ПолучитьIDСотрудников(Исполнитель);
		
		Для Каждого ПолучательID Из СписокПолучателей Цикл
			ТекстЗапроса = "/bot"+Токен + "/sendmessage?chat_id=" + Формат(ПолучательID,"ЧГ=") + "&text=" + ТекстОтвета;
			...
			Исключение
				...
				Возврат Новый Структура("Ответ,Ошибка",Ответ,Ошибка);
			КонецПопытки;
			//Ответ = ОтправитьНачальноеМеню(Токен,Подключение,ПолучательID).Ответ;
		КонецЦикла;

	КонецЕсли;		
КонецФункции

Функция ПолучитьIDСотрудников(Сотрудник)
	Запрос = Новый Запрос;	
