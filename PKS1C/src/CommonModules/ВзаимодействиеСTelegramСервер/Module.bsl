...
Функция ПолучитьКлавиатуруAcceptTask(ChatID=0,Заявка)
	...
	КнопкаAcceptTask.Вставить("text","Принять заявку");
...
Функция ПринятьЗаявку(Заявка,Исполнитель,ChatID) Экспорт
	...
	Если (Заявка.Статус = Перечисления.СтатусыЗаявок.Закрыта ИЛИ Заявка.Статус = Перечисления.СтатусыЗаявок.Отменена) И Не РазрешеноРедактированиеЗакрытыхЗаявок Тогда
