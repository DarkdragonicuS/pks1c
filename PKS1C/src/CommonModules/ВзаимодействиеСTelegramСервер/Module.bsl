...
Функция СформироватьДанныеПоЗаявкам(Исполнитель=Неопределено,Страница=Неопределено) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Сообщение = Строка(Номер) + ")" + "Исполнитель: " + ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ОтветственноеЛицо),ВыборкаДетальныеЗаписи.ОтветственноеЛицо,"<не указан>")
...
Функция ВыполнитьКомандуGetLicense(Токен,Отправитель,Подключение=Неопределено,ПараметрыКоманды=Неопределено)
	...
	Если ПараметрыКоманды = Неопределено Или ПараметрыКоманды.Количество() = 0 Тогда
		...
		Ответ = GetLicenseОтправитьЗапросКлиента(Токен,Отправитель,Подключение);			
	...
	Если КоличествоЗаписей > 0 Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Иначе
				...
				ПараметрыКнопки.Вставить("callback_data",ПолучитьДанныеКомандыGetLicense(ВыборкаДетальныеЗаписи.Ссылка,,,,Отправитель.id));
		...
		ПараметрыКнопки.Вставить("callback_data",ПолучитьДанныеКомандыGetLicense(ВыборкаДетальныеЗаписи.Ссылка,1,,,Отправитель.id));
		МассивКнопок.Добавить(ПараметрыКнопки);				
		...
		Если Страница > 1 Тогда
			ПараметрыКнопки = Новый Структура();
			ПараметрыКнопки.Вставить("text","<");
			ПараметрыКнопки.Вставить("callback_data",ПолучитьДанныеКомандыGetLicense(ВыборкаДетальныеЗаписи.Ссылка,Страница-1,,,Отправитель.id));
			МассивКнопок.Добавить(ПараметрыКнопки);
		КонецЕсли;
		...
		ПараметрыКнопки = Новый Структура();
		ПараметрыКнопки.Вставить("text",">>");
		ПараметрыКнопки.Вставить("callback_data",ПолучитьДанныеКомандыGetLicense(ВыборкаДетальныеЗаписи.Ссылка,ОкруглитьВерх(КоличествоЗаписей / 10),,,Отправитель.id));
		МассивКнопок.Добавить(ПараметрыКнопки);				
...
Функция ПолучитьДанныеКомандыGetLicense(Клиент=Неопределено,Страница=Неопределено,Номенклатура=Неопределено,Документ=Неопределено,Лицензия=Неопределено,ChatID=0)
	ДанныеКомандыОбъект = Новый Структура;
	ДанныеКомандыОбъект.Вставить("c","getLicense");
	ПараметрыКоманды = Новый Структура;
	//ПараметрыКоманды.Вставить("c","getLicense");
	...
	Если Не Номенклатура = Неопределено Тогда
		ПараметрыКоманды.Вставить("w",Номенклатура);
	КонецЕсли;
	ДанныеКомандыОбъект.Вставить("p",ПараметрыКоманды);
...
Функция РазослатьЗаявку(Заявка,Параметры,Токен=Неопределено,Подключение=Неопределено) Экспорт
	...
	СменаИсполнителя = Параметры.СменаИсполнителя;
...
Функция ПолучитьДанныеЛицензииПоИдентификатору(ЛицензияID) Экспорт
	//TODO
	//ЛицензияID = Новый УникальныйИдентификатор(ЛицензияID);
	Возврат Неопределено;
КонецФункции