	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		Если НоваяЗаявка Тогда
			Если ВыборкаДетальныеЗаписи.Приоритет = Перечисления.ПриоритетыЗаявок.Срочно Тогда
				СообщениеИсполнителю = "<b>СРОЧНО!</b>" + Символы.ПС;
			КонецЕсли;
		...
		Для Каждого ПолучательID Из СписокПолучателей Цикл
			...
			ТекстЗапроса = "/bot"+Токен + "/sendmessage?chat_id=" + Формат(ПолучательID,"ЧГ=") + "&text=" + ЭкранированиеСимволовQueryString(СообщениеИсполнителю) + "&parse_mode=HTML";
