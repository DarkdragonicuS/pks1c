...
Функция GetLicenseОтправитьЗапросКлиента(Токен,Отправитель,Подключение=Неопределено,Страница=1)
	...
	ПараметрыКнопки = Новый Структура();
	ПараметрыКнопки.Вставить("text",">>");
	ПараметрыКнопки.Вставить("callback_data",ПолучитьДанныеКомандыGetLicense(,,ОбщееКоличествоСтраниц,,,,Отправитель.id));
	МассивКнопок.Добавить(ПараметрыКнопки);				
...
// 
// Параметры:
//  Подкоманда - Число - определяет следующее действие команды: Неопределено - вывод клиентов для выбора, 1 - вывод
//  	номенклатуры для выбора, 2 - вывод реализация для выбора, 3 - вывод лицензий реализации, 4 - вывод лицензии, 5 -
//  	выбор страницы вывода
