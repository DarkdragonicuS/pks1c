	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		Если НоваяЗаявка Тогда
			СообщениеИсполнителю = 
				...
				+ Символы.ПС + "Исполнитель: " + ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ОтветственноеЛицо),ВыборкаДетальныеЗаписи.ОтветственноеЛицо,"<не указан>")
				+ Символы.ПС + "Клиент: " + ВыборкаДетальныеЗаписи.Клиент
				+ Символы.ПС + "Контакт: " + ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.КонтактноеЛицо),ВыборкаДетальныеЗаписи.КонтактноеЛицо + " - ","") + ?(ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.КонтактныйТелефон),ВыборкаДетальныеЗаписи.КонтактныйТелефон,"<телефон не указан>")
				+ Символы.ПС + "Тема: " + ВыборкаДетальныеЗаписи.ТемаЗаявки
				+ Символы.ПС + "---" + Символы.ПС + ПолучитьТекстЗаявки(ВыборкаДетальныеЗаписи.ТекстЗаявки);
		...
		Если СменаИсполнителя Тогда
			...
			Для Каждого ПолучательID Из СписокПолучателей Цикл
				ТекстЗапроса = "/bot"+Токен + "/sendmessage?chat_id=" + Формат(ПолучательID,"ЧГ=") + "&text=" + ЭкранированиеСимволовQueryString(СообщениеСтаромуИсполнителю);
		...
		Для Каждого ПолучательID Из СписокПолучателей Цикл
			...
			Если Не Контакт = Неопределено Тогда				
				КлавиатураGetContact = ПолучитьКлавиатуруGetContact(ПолучательID,КонтактИмя,КонтактныйТелефон);
				ПараметрыСообщения.Вставить("reply_markup",КлавиатураGetContact);
			КонецЕсли;
...
// Возвращает строку JSON с параметрами вывода запроса контакта
// 
// Параметры:
...
// 
// Возвращаемое значение:
//  Строка - JSON-строка с клавиатурой "reply_markup" с кнопкой "Получить контакт"
