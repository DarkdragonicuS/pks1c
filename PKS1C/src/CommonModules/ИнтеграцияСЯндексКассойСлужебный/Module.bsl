...
Функция ДанныеОснованияПлатежаДляСервиса_Номенклатура(Знач ДанныеОснованияПлатежа) 
	...
	Если ОтправлятьДанныеЧекаВФормате01072019() Тогда
		СоответствиеПолей.Вставить("КодТовара",                    "product_code");
		СоответствиеПолей.Вставить("КодСтраныПроисхожденияТовара", "country_of_origin_code");
		СоответствиеПолей.Вставить("НомерТаможеннойДекларации",    "customs_declaration_number");
...
Функция ДанныеОснованияПлатежаДляСервиса_Чек(Знач ДанныеОснованияПлатежа) 
	...
	Если ОтправлятьДанныеЧекаВФормате01072019() Тогда
		Чек.Вставить("customer", ДанныеОснованияПлатежа_Покупатель(ДанныеОснованияПлатежа.Покупатель));
	КонецЕсли;
...
Функция ДанныеОснованияПлатежа_Покупатель(ДанныеПокупателя)
	...
	Результат.Вставить("full_name", ДанныеПокупателя.Наименование);
	Результат.Вставить("inn",       ДанныеПокупателя.ИНН);
	Результат.Вставить("email",     "");
	Результат.Вставить("phone",     "");
	
	Если ЭтоАдресПочты(ДанныеПокупателя.КонтактныеДанныеЧека) Тогда
		Результат.email = ДанныеПокупателя.КонтактныеДанныеЧека;
	Иначе
		Результат.phone = ДанныеПокупателя.КонтактныеДанныеЧека;
	КонецЕсли;
