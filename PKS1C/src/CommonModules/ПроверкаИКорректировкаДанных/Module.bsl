...
Функция ПроверитьДанныеОбщая(Проверки, Исправлять, СохранятьРезультат, Адрес = Неопределено)
	...
	Комментарий = СтрШаблон(НСтр("ru = 'Количество проверок: %1.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), КоличествоПроверок);
	...
	Если Не ТребуетсяЗаполнитьНастройки(Проверки, Исправлять, Обработчики) Тогда
		...
		Для Каждого Проверка Из Проверки Цикл
			...
			Комментарий = НСтр("ru = 'Начало выполнения проверки.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
			...
			Комментарий = СтрШаблон(НСтр("ru = 'Результат проверки: %1.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), РезультатПроверки.ПредставлениеРезультата);
			ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация, , , Комментарий);
	...
	Если Результат.Выполнено Тогда
		Комментарий = НСтр("ru = 'Проверки выполнены.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	Иначе
		Комментарий = НСтр("ru = 'Требуется заполнение настроек'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция ШаблонОшибкиВыполненияПроверки()
	...
	Возврат НСтр("ru = 'Не удалось выполнить проверку по причине: %1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЗапускЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Запуск'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЗавершениеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Завершение'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЗаписьИсторииЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Запись истории'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеПроверкиЖурналаРегистрации(Проверка)
	...
	Возврат СтрШаблон(НСтр("ru = 'Проверка и корректировка данных.%1'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Проверка);
